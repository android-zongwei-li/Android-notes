...
Kotlin 协程提供了一种全新的处理并发的方式，可以简化异步执行的代码。Google 官方推荐将 Kotlin 协程作为在 Android 上进行异步编程的解决方案，值得关注的功能点包括：

- 轻量：可以在**单个线程上运行多个协程**，因为协程支持挂起，不会使正在运行协程的线程阻塞。挂起比阻塞节省内存，且支持多个并行操作
...
在 Android 中引用以下的 coroutines-android，当中已经包含了 coroutines-core
