...
这个例子的结果result是：
![image.png](images/【总结篇】synchronized/1240.webp)

![image.png](images/【总结篇】synchronized/1240-1722388264615-3.webp)
...
关键字synchronized可以保证**在同一时刻，只有一个线程可以执行某个方法或者某个代码块**，这样同一时刻就只有一个线程访问方法或者代码块中用到的共享数据；同时synchronized可以**保证一个线程的变化可见**（可见性），即可以代替volatile。
...
一个线程持有一个 Monitor 列表，一个 Monitor 中会记录当前被哪个线程持有。加锁后，只有持有对象 Monitor 的线程才可以访问同步代码。
...
![image.png](images/【总结篇】synchronized/1240-1722389703378-6.webp)

分析：

当两个并发线程（t1 和t2）同时访问同一个对象（synchronizedTest）中的synchronized代码块时，在同一时刻只能由一个线程得到执行，另一个线程受阻塞，必须等待当前线程执行完这个代码块以后其他线程才能执行此代码块。t1和t2是互斥的，因为在执行synchronized代码块时会锁定当前的对象，只有执行完该代码块才能释放该对象锁，下一个线程才能执行并锁定该对象。
...
![image.png](images/【总结篇】synchronized/1240-1722390194587-9.webp)
...
![image.png](images/【总结篇】synchronized/1240-1722390198208-12.webp)
...
![image.png](images/【总结篇】synchronized/1240-1722390399761-15.webp)
...
> todo：新建一个对象作为锁，和直接用 Test 对象作为锁有啥区别吗，最终的效果应该是一样的吧？有区别的话，怎么选择合适的用法？
...
![image.png](images/【总结篇】synchronized/1240-1722390924296-18.webp)
