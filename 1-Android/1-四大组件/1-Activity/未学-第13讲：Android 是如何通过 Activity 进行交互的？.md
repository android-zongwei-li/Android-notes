...
![1.png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/Ciqc1F6qoXmACSfeAACHGnopIuM983.png)
...
![2.png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/Ciqc1F6qoZ6ABVdcAAfaoaqmoBo490.png)

上图中的 TaskRecord 代表一个任务栈，在这个栈中存在两个 Activity 实例：First 和 Second，并且 Second 处于栈顶。

接下来将 Second 的 taskAffinity 修改一下，如下所示：

![3.png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/Ciqc1F6qoaeARMUkAACDSLIIs3Y651.png)

我将 Second 的 taskAffinity 修改为 ”lagou.affinity“，使它和 First 的 taskAffinity 不同。重新运行代码，并再次查看任务栈中的情况，结果如下：

![image (1).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qobyAOxwAAAfcm-3M6WU365.png)
...
![image (2).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qocWAAL2tAAjjzHorETA263.png)
...
![image (3).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qofaAPOEYAAEyPzU2-7U272.png)

接下来，打开 TaskAffinityReparent 项目，屏幕上本应显示 ReparentActivity 的页面内容，但是实际上显示的却是 FirstC 中的页面内容，并且系统中 Activity 信息如下：

![image (4).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qoi6AYBKbAAbxnNqle-w449.png)
...
![image (5).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qox6APKSiAAFYCDL9YmU374.png)


在 startFirstB 方法中，跳转 FirstB 页面，并通过 Intent 传递 Bean 类中的数据。但是执行上述代码会报如下错误：

![image (6).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qoxaAPCzSAALV6Tx3gOc932.png)
...
![image (7).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qo36AE1wYAAFjocGh-yY383.png)
...
![image (8).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qo-OAVcSFAAF8T_A2nPc539.png)

但实际上，Activity 可以在不同的进程中启动，而每一个不同的进程都会创建出一个 Application，因此有可能造成 Application 的 onCreate 方法被执行多次。比如以下代码：

![image (9).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qo_SAHEpIAALroorEfrs345.png)
...
![image (10).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/Ciqc1F6qo_2AXKfoAAKiJ356was666.png)

最终打印日志如下：

![image (11).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qpAaAH3qxAAClcmfTjiA226.png)
...
![image (12).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qpkeADW1nAAGdO7dLZfk559.png)
...
![image (13).png](images/未学-第13讲：Android 是如何通过 Activity 进行交互的？/CgqCHl6qpmaAZwATAAFGPPvXtn4262.png)
